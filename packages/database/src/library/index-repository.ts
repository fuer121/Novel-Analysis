...
export function createIndexRepository(db: DatabaseExecutor, cipher: ContentCipher) {
  return {
    ...
    createWorkflowVersion(input: { target: "chapter-import" | "l1-index" | "l2-index" | "analysis-summary"; contractVersion: string; dslHash: string }) { return db.insertInto("workflow_versions").values({ target: input.target, contract_version: input.contractVersion, dsl_hash: input.dslHash }).returningAll().executeTakeFirstOrThrow(); },
