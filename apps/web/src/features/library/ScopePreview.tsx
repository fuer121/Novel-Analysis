import type { ImportScopePreview, IndexCoverage, ScopePreview as ScopePreviewValue } from "./types.js";
...
export function ImportPreviewStrip({ value }: { value: ImportScopePreview }) {
  return <dl className="scope-strip import-preview-strip"><div><dt>请求</dt><dd>{value.requested}</dd></div><div><dt>将执行</dt><dd>{value.executable}</dd></div><div><dt>将跳过</dt><dd>{value.requested - value.executable}</dd></div><div><dt>已有新鲜内容</dt><dd>{value.existingFresh}</dd></div><div><dt>已有过期内容</dt><dd>{value.existingStale}</dd></div></dl>;
}
