...
export function L2Panel() {
  ...
  if (groups.isError) return <div className="workspace-section"><div className="error-notice">索引组读取失败 <button className="text-button" type="button" onClick={() => void groups.refetch()}>重试索引组</button></div></div>;
  if (!groups.data?.indexGroups.length) return <div className="workspace-section"><p className="empty-state">尚未配置可用的 L2 索引组</p><button className="secondary-button" type="button" onClick={() => void groups.refetch()}>刷新索引组</button></div>;
  const scope = { startChapter, endChapter, mode, force: false };
  return <div className="workspace-section l2-layout"><div className="l2-controls"><label>索引组<select value={groupId} onChange={(event) => setGroupId(event.target.value)}>{groups.data.indexGroups.map((group) => <option key={group.id} value={group.id}>{group.name}</option>)}</select></label>{coverage.isPending ? <p className="empty-state">正在读取 L2 覆盖情况...</p> : null}{coverage.isError ? <div className="error-notice">L2 覆盖情况读取失败 <button className="text-button" type="button" onClick={() => void coverage.refetch()}>重试 L2 覆盖情况</button></div> : null}{coverage.data ? <CoverageStrip value={coverage.data} /> : null}
