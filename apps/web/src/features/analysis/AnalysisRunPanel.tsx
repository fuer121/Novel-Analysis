...
export function AnalysisRunPanel({ run, pending, onControl, onDelete }: Props) {
  ...
  return <section className="analysis-run-detail" aria-label="任务详情">
    ...
    <div className="analysis-parts"><h3>执行步骤</h3>{run.parts.length ? run.parts.map((part) => <div key={part.id}><span>位置 {part.position}</span><strong>{part.kind}</strong><span className={`status status-${part.status}`}>{part.status}</span>{part.errorCode ? <code>{part.errorCode}</code> : null}</div>) : <p className="analysis-empty">尚未生成执行步骤</p>}</div>
