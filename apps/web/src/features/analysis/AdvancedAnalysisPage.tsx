...
function RunCreator({ book, template, groups, onCreated }: { book: BookSummary; template: AnalysisTemplateDetail; groups: IndexGroup[]; onCreated: (runId: string) => void }) {
  ...
  const preview = useMutation({ mutationFn: () => previewAnalysis(book.id, { bookId: book.id, templateId: template.id, mode, startChapter, endChapter }), onSuccess: (value) => { attempt.current = { fingerprint: JSON.stringify(value), key: crypto.randomUUID() }; setMessage(null); }, onError: (error) => setMessage(error instanceof ApiError && error.status === 404 ? "模板或范围不存在" : "范围预览失败，请重试") });
  const create = useMutation({
    mutationFn: (scope: AnalysisScopePreview) => {
      const fingerprint = JSON.stringify(scope);
      if (attempt.current?.fingerprint !== fingerprint) attempt.current = { fingerprint, key: crypto.randomUUID() };
      return createAnalysisRun(book.id, { bookId: book.id, templateId: template.id, templateVersionId: scope.templateVersionId, mode: scope.mode, startChapter: scope.startChapter, endChapter: scope.endChapter, scopeHash: scope.scopeHash, idempotencyKey: attempt.current.key });
    },
    onSuccess: async ({ run }) => { attempt.current = null; setMessage(null); await client.invalidateQueries({ queryKey: analysisKeys.runs(book.id) }); onCreated(run.id); },
    onError: (error) => {
      if (error instanceof ApiError && error.code === "scope_changed") { preview.reset(); attempt.current = null; setMessage("范围已变化，请重新预览"); return; }
      setMessage(error instanceof ApiError && error.code === "idempotency_conflict" ? "创建请求与先前内容冲突，请重新预览" : "提交结果未确认，请使用同一范围重试");
    },
  });
  const invalidatePreview = () => { preview.reset(); attempt.current = null; setMessage(null); };
  ...
  return <section className="analysis-creator" aria-label="创建分析任务">
    ...
    <div className="analysis-scope-controls"><label>分析模式<select value={mode} onChange={(event) => { setMode(event.target.value as AnalysisMode); invalidatePreview(); }}>{Object.entries(modeLabels).map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label><label>起始章节<input type="number" min="1" max={Math.max(1, book.chapterCount)} value={startChapter} onChange={(event) => { setStartChapter(Number(event.target.value)); invalidatePreview(); }} /></label><label>结束章节<input type="number" min="1" max={Math.max(1, book.chapterCount)} value={endChapter} onChange={(event) => { setEndChapter(Number(event.target.value)); invalidatePreview(); }} /></label></div>
...
export function AdvancedAnalysisPage() {
  ...
  const [awaitingResumeRunId, setAwaitingResumeRunId] = useState<string | null>(null);
  ...
  const run = useQuery({ queryKey: analysisKeys.run(book.id, selectedRunId ?? "none"), queryFn: () => readAnalysisRun(book.id, selectedRunId!), enabled: view === "new" && Boolean(selectedRunId), refetchInterval: (query) => query.state.data && (["queued", "running", "retrying"].includes(query.state.data.run.status) || query.state.data.run.status === "paused" && awaitingResumeRunId === selectedRunId) ? 3_000 : false });
  ...
  const control = useMutation({ mutationFn: (action: "pause" | "resume" | "cancel") => controlAnalysisRun(run.data!.run.jobId, action), onMutate: (action) => setAwaitingResumeRunId(action === "resume" ? selectedRunId : null), onSuccess: async () => { await Promise.all([client.invalidateQueries({ queryKey: analysisKeys.run(book.id, selectedRunId!) }), client.invalidateQueries({ queryKey: analysisKeys.runs(book.id) })]); }, onError: () => setAwaitingResumeRunId(null) });
  ...
  useEffect(() => {
    if (awaitingResumeRunId && (selectedRunId !== awaitingResumeRunId || run.data?.run.status !== "paused")) setAwaitingResumeRunId(null);
  }, [awaitingResumeRunId, run.data?.run.status, selectedRunId]);
  ...
  return <section className="workspace-section advanced-analysis-page">
    ...
    <div className="analysis-toolbar"><div className="analysis-segments" role="tablist" aria-label="分析视图"><button type="button" role="tab" aria-selected={view === "new"} onClick={() => { setParam("view", null); setDrawerOpen(false); }}>新任务</button><button type="button" role="tab" aria-selected={view === "legacy"} onClick={() => { setParam("view", "legacy"); setDrawerOpen(false); }}>旧历史</button></div><button className="secondary-button analysis-list-trigger icon-command" type="button" onClick={() => setDrawerOpen(true)}><List size={16} />{view === "new" ? "模板与任务" : "旧历史列表"}</button></div>
