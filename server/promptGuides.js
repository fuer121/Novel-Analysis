...
const guideDefinitions = {
  l1: {
    ...
    scope: "书籍级索引 Prompt",
    positioning: "L1 是轻量章节路由/信号索引，服务所有 L2 索引组和后续二次分析。它应长期绑定一本书，构建完成后不频繁调整。",
    steps: [
      {
        id: "routing_scope",
        title: "范围",
        question: "这本书后续最需要 L1 帮你命中的内容方向是什么？",
        helper: "只写核心方向和典型对象，不需要设计字段。L1 会服务所有 L2 索引组。",
        placeholder: "需要能命中人物出场与身份变化、外貌变化、关系变化、修炼体系、法宝武器、宗门势力和伏笔线索。"
      },
      {
        id: "routing_rules",
        title: "取舍",
        question: "哪些信号必须保留，哪些内容不要进入 L1？",
        helper: "写清楚命中优先级和排除项，避免 L1 膨胀成事实库。",
        placeholder: "必须保留首次出场、身份/关系/外貌/境界/物品变化和重要伏笔；普通流水账、短暂情绪、无后续价值的打斗过程不要进入 L1；低置信判断要标注。"
      }
    ],
    builtInPrompt: [
      "你要为一本长篇小说生成 L1 章节路由/信号索引构建 Prompt。",
      "L1 的定位是轻量章节路由和召回入口，服务所有 L2 索引组，不是深度事实库，也不是最终分析结果。",
      "分析阶段会先按章节顺序扫描 L1，命中目标主体、分类词或路标信号后，再读取对应章节的 L2 事实；因此 L1 必须使用稳定、可检索的词表。",
      "生成的 Prompt 必须要求模型只依据当前章节原文，避免补全和脑补。",
      "Prompt 应指导模型提取短路由摘要、主体/别名、关键词、分类信号、类别分数、重大信号和置信度；这些字段应优先承载后续分析会查询的主体、别名、分类词和变化信号。",
      "Prompt 要控制输出密度，优先保留后续可被召回的高价值信号；不要把 L1 写成 L2 事实清单。",
      "Prompt 应明确：不要输出 Markdown，不要引用长段原文，不要把无关流水账写入索引。",
      "Prompt 应适合绑定到单本书长期使用，修改后会导致旧索引过期。",
      "生成的 L1 Prompt 应短而明确，包含：定位、主体/别名规则、每章路由信号优先级、分类分数规则、密度控制、禁止项和不确定性标注。"
    ].join("\n")
  },
  l2: {
    type: "l2",
    ...
    steps: [
      {
        id: "fact_scope",
        title: "范围",
        question: "这个 L2 Prompt 需要提取哪类可复用事实？",
        helper: "如果是 base，就写通用事实边界；如果是专项组，就只写该组负责的内容。",
    ...
    builtInPrompt: [
      ...
      "生成的 L2 Prompt 应短而明确，包含：事实范围、主体/别名/标签规则、原子事实拆分规则、证据与置信度规则、重要度规则、排除项和与 L1 词表对齐要求。"
    ].join("\n")
  },
  indexgroup: {
    type: "indexGroup",
    label: "索引组创建引导",
    scope: "书籍级 L2 专项索引组",
    positioning: "索引组用于把单一 L2 的内容压力拆开。每个专项组只负责一类稳定分析诉求，共用 L1 章节路由，再按需构建本组 L2 事实。",
    steps: [
      {
        id: "group_goal",
        title: "用途",
        question: "这个索引组专门负责哪类分析诉求？",
        helper: "只写一类稳定方向，避免把多个大方向塞进同一个专项组。",
        placeholder: "负责修炼体系、境界变化、功法传承、法宝武器和本命物相关事实，供后续分析修炼设定和物品设定时召回。"
      },
      {
        id: "group_boundaries",
        title: "边界",
        question: "它应该提取什么、忽略什么、用哪些触发词自动命中？",
        helper: "写清楚触发词、事实边界和排除项。",
        placeholder: "触发词包括修炼、境界、功法、法宝、武器、本命物、血脉；只提取长期设定和明确变化，不记录普通打斗过程、临时招式效果和纯气氛描写。"
      }
    ],
    builtInPrompt: [
      "你要为一本长篇小说设计一个 L2 专项索引组，并生成该组的 L2 Prompt。",
      "索引组的定位是拆分单一 L2 的内容压力；每组只负责一类稳定分析诉求，共用 L1 章节路由，不重建独立 L1。",
      "生成结果必须帮助用户填写索引组草稿：标题建议应是简短组名，Prompt 建议应是该专项组的 L2 构建 Prompt。",
      "Prompt 必须明确本组只提取哪些事实、触发词/分类词如何与 L1 route 对齐、主体/别名/标签如何记录、事实颗粒如何拆分、证据/重要度/置信度如何处理、哪些内容不要进入本组。",
      "不要把多个互不相关的大方向合并成一个组；如果用户诉求过宽，应在使用提示中建议拆成多个专项组。",
      "Prompt 不要要求重新构建 L1，不要要求逐章分析最终结论，不要输出 Markdown。"
    ].join("\n")
  },
  analysis: {
    type: "analysis",
