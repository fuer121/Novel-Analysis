...
6. 仅在步骤5通过后才可启动allowlisted target-preflight child，核验target asset binding、server profile freshness、clock、exclusive window、network isolation与idle load；child退出后必须证明child及其全部descendant process absent，并再次证明observed current-run inventory与该阶段允许保留的directories、session、launcher、sinks及preflight output exact match，target-preflight child identity不得继续存在
...
| Target preflight | `TARGET_ASSET_MISMATCH`、`TARGET_PROFILE_STALE`、`TARGET_WINDOW_CLOSED`、`TARGET_CLOCK_UNTRUSTED`、`TARGET_FORMAL_TRAFFIC_PRESENT`、`TARGET_PUBLIC_ENTRY_PRESENT`、`TARGET_EXCLUSIVE_ISOLATION_UNPROVEN`、`TARGET_LOAD_NOT_IDLE`、`TARGET_PREFLIGHT_PROCESS_REMAINS`、`TARGET_PREFLIGHT_INVENTORY_MISMATCH` | Snapshot与key未请求 |
...
| Key | `KEY_DELIVERY_FAILED`、`KEY_OWNER_MISMATCH`、`KEY_MODE_MISMATCH`、`KEY_SYMLINK`、`KEY_LENGTH_INVALID`、`KEY_NOT_DISTINCT`、`KEY_INVENTORY_MISMATCH` | Migration未启动 |
...
| Post-migration teardown | `KEY_DESTRUCTION_EVIDENCE_MISSING` | Capacity未启动 |
...
| Pre-execution cleanup contract | `CLEANUP_TARGET_MISSING`、`OUTPUT_CLEANUP_UNMAPPED`、`OUTPUT_CLEANUP_DUPLICATED`、`ABSOLUTE_DEADLINE_INVALID` | Current-run object与target access未创建，Execution Gate保持locked |
| Post-run cleanup and review | `ACCESS_REVOCATION_FAILED`、`CUSTODY_EXPIRED`、`SEALED_INVENTORY_MISMATCH`、`POST_RUN_SPEC_REVIEW_MISSING`、`POST_RUN_SPEC_REVIEW_FAILED`、`POST_RUN_QUALITY_REVIEW_MISSING`、`POST_RUN_QUALITY_REVIEW_FAILED`、`POST_RUN_REVIEW_TIMEOUT`、`DEADLINE_CLEANUP_FAILED`、`PROCESS_ABSENCE_FAILED`、`FILE_ABSENCE_FAILED`、`KEY_ABSENCE_FAILED`、`LOCAL_TCP_ABSENCE_FAILED`、`TASK_RUNTIME_ABSENCE_FAILED` | Result Gate保持locked且不得补跑或延长custody |
...
- Exact atomic case inventory为`134`个unique IDs，其中`133`个failure IDs与`1`个success-state ID `PUBLICATION_VALID`
