...
- `/prompts`：模板与事实索引管理台
  ...
  - 增加事实索引管理：默认显示“默认事实索引”，支持按需创建多个事实索引并编辑名称/用途/触发词/事实索引规则
...
- 产品层不再展示 `base` / “基础索引”。内部 `base` 仍作为隐藏兼容 key 长期保留，历史 L2 状态和事实迁移或读取时默认归属 `base`，UI 统一显示为“默认事实索引”。
...
- 分析 Prompt 可显式绑定一个或多个事实索引；未绑定时按事实索引触发词和范围自动推断，推断不到则回退内部 `base`，用户侧显示为“默认事实索引”。
...
- 书籍级兼容 Prompt 保存在 `book_index_prompts`，和 `books.book_id` 一一对应；每本书有一套当前生效的 L1 Prompt 和内部 `base` L2 Prompt，UI 展示为“默认事实索引”的事实索引规则。
- 自定义事实索引 Prompt 保存在 `book_index_groups.l2_index_prompt`，只影响该事实索引，不影响内部 `base` 或其他索引。
- `/prompts` 是统一模板与事实索引管理台，管理当前书籍的 L1、默认事实索引、自定义事实索引和分析 Prompt。
...
- 保存自定义书籍级 L1/内部 `base` L2 Prompt 后，Hash 变为基于该书 Prompt 内容计算的 SHA-256；后续构建任务会按新 Hash 跳过、重建或标记过期。
